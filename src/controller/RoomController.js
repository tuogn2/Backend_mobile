...
class roomController {
  ...
  async addRoom(req, res) {
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ message: "Vui lòng cung cấp đầy đủ thông tin phòng." });
      }
      ...
      res
        .status(201)
        .json({ message: "Thêm phòng thành công!", room: newRoom });
    } catch (error) {
    ...
    try {
      ...
      if (!location || !startDate || !endDate) {
        return res.status(400).json({
          message:
            "Vui lòng cung cấp location, startDate và endDate để tìm kiếm.",
        });
      }
      ...
      if (availableRooms.length === 0) {
        return res.status(404).json({
          message:
            "Không tìm thấy phòng trống với vị trí và khoảng thời gian đã chọn.",
        });
      }
  ...
  async getRoomsByIds(req, res) {
    try {
      ...
      if (rooms.length === 0) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy phòng nào với danh sách ID này." });
      }
  ...
  async getRoomById(req, res) {
    try {
      ...
      if (!room) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy phòng với ID này." });
      }
  ...
  async getRoomsByType(req, res) {
    try {
      ...
      if (rooms.length === 0) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy phòng với thể loại này." });
      }
  ...
  async searchRoomsByAreaAndPriceRange(req, res) {
    try {
      let { typeArea, prePrice, postPrice } = req.query; // Chuyển 'const' thành 'let'

      const searchConditions = {}; // Đây không phải là vấn đề, vì searchConditions không bị thay đổi

      // Kiểm tra và xử lý typeArea
      if (typeArea) {
        typeArea = typeArea.trim(); // Sửa dòng này: chuyển từ 'const' thành 'let'
        searchConditions.typeArea = { $regex: new RegExp(typeArea, "i") };
      }

      // Kiểm tra và xử lý prePrice, postPrice
      if (prePrice && postPrice) {
        const minPrice = parseFloat(prePrice);
        const maxPrice = parseFloat(postPrice);

        // Kiểm tra nếu giá trị prePrice nhỏ hơn postPrice
        if (minPrice > maxPrice) {
          return res.status(400).json({
            success: false,
            message: "Giá trị prePrice phải nhỏ hơn postPrice.",
          });
        }
        searchConditions.price = { $gte: minPrice, $lte: maxPrice };
      }
      ...
      if (rooms.length === 0) {
        return res.status(404).json({
          message: "Không tìm thấy phòng với các tiêu chí đã chọn.",
        });
      }

      res.status(200).json({ success: true, data: rooms });
    } catch (error) {
      console.error("Error searching rooms by area and price range:", error);
      res.status(500).json({
        success: false,
